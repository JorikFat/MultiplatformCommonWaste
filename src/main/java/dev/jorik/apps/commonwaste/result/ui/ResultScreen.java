package dev.jorik.apps.commonwaste.result.ui;
...
import dev.jorik.apps.commonwaste.result.presentation.ResultView;
...
public class ResultScreen implements ResultView {
    ...
    public void showResults(List<Person> persons, List<Waste> wastes) {
        ...
        for(Waste waste :wastes){
            ...
            for (Person consumer :persons){
                if(consumer == waste.getPayer()){
                    System.out.print(format(waste.getAmount(), 10, "-"));
                } else if(waste.getConsumers().contains(consumer)) {
